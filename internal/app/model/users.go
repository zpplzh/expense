...
var (
	userAllColumns            = []string{"user_id", "email", "password", "created_at", "updated_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"user_id", "email", "password", "created_at", "updated_at", "deleted_at"}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, userID string, selectCols ...string) (*User, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"users\" where \"user_id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *User) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"users\" SET %s WHERE \"user_id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, userID string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"users\" where \"user_id\"=$1 and \"deleted_at\" is null limit 1)"
