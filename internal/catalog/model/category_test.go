...
func testCategoriesSoftDelete(t *testing.T) {
	t.Parallel()
...
func testCategoriesQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Categories().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testCategoriesSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testCategoriesQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Categories().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
var (
	categoryDBTypes = map[string]string{`Categoryid`: `text`, `Name`: `text`, `Icon`: `text`, `UserID`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_               = bytes.MinRead
)
